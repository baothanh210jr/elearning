<template>
    <header class="flex flex-col sticky top-0 left-0 w-full z-50">
        <div
            style="height: var(--header-height)"
            class="text-black px-8 flex justify-between items-center bg-primary"
        >
            <!-- Logo and Navigation -->
            <NuxtLink class="flex items-center space-x-4" to="/">
                <!-- Logo -->
                <div
                    class="text-xl font-bold cursor-pointer b b-b-green-400 b-b-opacity-50 p-2 text-white"
                >
      ...
      > -->
            ...
            <div v-else>
                <div class="flex items-center gap-5">
                    <UAvatar
                        :src="`${config.public.apiBase}/assets/${applicationStore.user?.avatar}`"
                    />
                    <UButton
                        @click="handleLogout"
                        loading-auto
        ...
        <SidebarDesktop class="bg-gray-100 py-2" />
    </header>
</template>
